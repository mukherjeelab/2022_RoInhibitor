...
## Compare Ro sensitivity patterns between construct and dox treatment


```{r no dox, warning=FALSE, message=FALSE}
rd4 <- read_xls(here("data","K562Viability", "ECH003_4_05042022.xls"))
...
#looking at the constructs by Dox treatment. 
ggline(data = rd4_long, x = "Ro", y = "cytotox", color = "construct", facet.by = "dox", numeric.x.axis = F,
 add = "mean_se")

#looking at the dox treatment within each construct. 
...
## Normalizing no dox treatment


```{r no dox norm, warning=FALSE, message=FALSE}
...
## Normalizing the Dox treatment


```{r dox norm, warning=FALSE, message=FALSE}
...
all_normDox$value <- all_normDox$value*100


celltiterall <- ggline(data = all_normDox, x = "conc", y = "value", color = "construct", add = "mean_se",palette = c("red", "blue", "darkred", "darkblue"), numeric.x.axis = F) + ylab("% Viability") + xlab("[Ro] uM") +  theme(legend.position="bottom")
ggsave(plot = celltiterall, filename = here("plots", "Fig2Supp_K562_all.pdf"), width = 5, height = 4, bg = "white")
celltiterall

celltiterDox <- ggline(data = all_normDox %>% filter(str_detect(variable, "R100A")), x = "conc", y = "value", color = "construct", add = "mean_se", palette = c("red", "black"), numeric.x.axis = F) + ylab("%Viability") + xlab("Ro Concentration (uM)") +  theme(legend.position="bottom")
ggsave(plot = celltiterDox, filename = here("plots", "Fig2_K562_R100Aonly_percent_sd.pdf"), width = 3.5, height = 3, bg = "white")
celltiterDox
```
