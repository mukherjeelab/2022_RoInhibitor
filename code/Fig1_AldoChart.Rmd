...
OD1 <- read_csv(here("data", "ELISA", "20190719_Ro08-2750Titration_ELISA.csv"))
colnames(OD1) <- c("Sample","Well","OD_450","OD_540","OD_570")

pb1 <- read_csv(here("data", "ELISA", "20190718_Ro08-2750Titration_PB1.csv"))[,2:3]
...
OD2 <- read_csv(here("data", "ELISA", "20200131_Ro08-2750Titration_ELISA.csv"))
colnames(OD2) <- c("Sample","Well","OD_450","OD_540","OD_570")

pb2 <- read_csv(here("data", "ELISA", "20200130_Ro08-2750Titration_PB1.csv"))[,2:3]
colnames(pb2)[2] <- c("viability_24")

fr2 <- read_csv(here("data", "ELISA", "20200131_Ro08-2750Titration_FR.csv"))
...
OD3 <- read_csv(here("data", "ELISA", "20200305_Ro08-2750Titration_ELISA.csv"))
colnames(OD3) <- c("Sample","Well","OD_450","OD_540","OD_570")

pb3 <- read_csv(here("data", "ELISA", "20200304_Ro08-2750Titration_PB1.csv"))[,2:3]
colnames(pb3)[2] <- c("viability_24")

fr3 <- read_csv(here("data", "ELISA", "20200305_Ro08-2750Titration_FR.csv"))
...
## Stimulated, Ro Treated Cells Normalization to DMSO Control
...
#calculating percent aldosterone in the stimulated cells compared to DMSO control.
...
#calculating percent viability in the stimulated cells compared to DMSO control.
...
#calculating percent cell number in the stimulated cells compared to DMSO control. 
...
## Unstimulated, Ro Treated Cells Normalization to DMSO Control


```{r unstim data, include=TRUE, warning=FALSE, message=FALSE}

#calculating percent aldosterone in the unstimulated cells compared to DMSO control.
...
#calculating percent viability in the unstimulated cells compared to DMSO control.
...
colnames(allData) <- c("tx", "stim", "Well", "OD_450", "OD_540", "OD_570", "PB_24", "FR_24", "corrected", "loganswer", "conc pg/mL", "below Std Curve", "above Std Curve", "Ro_conc", "batch")

write.xlsx(allData, here("output", "AldoELISAvalues.xlsx"), sheetName = "Sheet1", col.names = TRUE)
```


## Plotting Stimulated Data


```{r stim plots IC50, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}

#making plots for each of the stimulated categories (% aldo, % viability, %DNA)
...
table(tmp$Ro_conc)
p_Aldo_per
#ggsave(plot = p_Aldo_per, filename = "./plots/Fig1_Aldo_percent_sd.pdf", width = 3, height = 4)
...
p_PB_per
#ggsave(plot = p_PB_per, filename = "./plots/Fig1_PB_percent_sd.pdf", width = 3, height = 4)
...
p_FR_per
#ggsave(plot = p_FR_per, filename = "./plots/Fig1_FR_percent_sd.pdf", width = 3, height = 4)

tmp$stim <- "stim_Ang"
tmp_us$stim <- "unstim_Ang"
allData <- rbind(tmp, tmp_us)
allData <- allData %>% select(c("stim", "batch", "pPB", "Ro_conc"))

OD5stim$batch <- "D"
OD5us$batch <- "E"
OD5stim$stim <- "stim_FSK"
OD5us$stim <- "unstim_FSK"
allData <- rbind(allData, OD5stim %>% select(c("stim", "batch", "pPB", "Ro_conc")), OD5us %>% select(c("stim", "batch", "pPB", "Ro_conc")))
#allData <- unite(allData, "batch", c("stim","batch"))

ggline(allData, x = "Ro_conc", y = "pPB", ylab = "% Viability", add = "mean_sd", xlab = "[Ro] uM", color = "stim") + rotate_x_text(60) + geom_hline(yintercept = 100, linetype = "dotted", color = "grey") + scale_fill_colorblind()
```


## IC50 Values Stimulated cells


```{r IC50 stimulated, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}
##Now calculating IC50 values for each of the above categories. Using a 4 parameter logistic model. 

#Fit (IC50 calculation) for pAldo
tmp2 <- tmp %>% select(Ro_conc, batch, conc)
tmp2 <- aggregate(conc~batch+Ro_conc,tmp,mean)

fit <- drm(formula =   pAldo ~ Ro_conc , data = tmp, fct=LL.4(fixed=c(NA, NA, NA, NA), names = c("Slope", "Lower Limit", "Upper Limit", "ED50")))
...
#Fit for pPB
fit <- drm(formula =   pPB ~ Ro_conc , data = tmp, fct=LL.4(names = c("Slope", "Lower Limit", "Upper Limit", "ED50")))
summary(fit)
plot(fit, type = "all")
...
## Plotting Unstimulated Data


```{r UNstim plots and IC50, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}

#making plots for each of the unstimulated categories (% aldo, % viability, %DNA)
...
ggsave(plot = p_all_PR_us, filename = here("plots", "Fig1Supp_Unstim_AldoELISA.pdf"), width = 6, height = 3)

```


## IC50 Unstimulated cells


```{r IC50 unstimulated, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}
