...
resTC %>% as.data.frame() %>% filter(symbol %in% c("SCD","LIPE","CYP11A1","HSD3B2","CYP21A2","CYP11B1","CYP11B2"))
```
...
p_pathway <- plot_grid(p_SCD  + theme(legend.position="none"),
                       p_HSL  + theme(legend.position="none"),
...
#ggsave(plot = p_pathway, device = "pdf", width = 3, height = 5, filename = here("plots","Fig1_RNAseq_FC.pdf"))
...
pheatmap(mat = clustmsigdb, scale = "none", cluster_cols = F, border_color = "black",  clustering_distance_rows = "euclidean", clustering_method = "complete", color = viridis(10, option = "E", direction = 1), filename = here("plots","BP_up_down.pdf"), width = 40, height = 10, fontsize = 24)
