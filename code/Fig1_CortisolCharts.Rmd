...
OD <- read_csv(here("data", "ELISA", "KAB039_Cortisol_ELISAall2.csv"))
...
## Normalizing to DMSO Control


```{r normalizing to DMSO, include=TRUE, warning=FALSE, message=FALSE}
#correcting for dilutions done in the assay - only going to use dilution 5 as this is within the standard range for the assay. 
OD$conc2 <- OD$conc * as.numeric(OD$dil)
OD1 <- OD %>% filter(OD$dil != 5)
OD5 <- OD %>% filter(OD$dil != 1)

#only using 1:5 dilution
OD5$Ro_conc <- OD5$tx
OD5$Ro_conc <- gsub(x = OD5$Ro_conc, pattern = "uM", replacement = "")
OD5$Ro_conc <- as.numeric(gsub("DMSO","0", OD5$Ro_conc))

#normalizing to DMSO for cortisol amounts
...
#normalizing to DMSO for presto blue assay
...
colnames(OD5) <- c("tx", "dil", "stim", "Well", "OD_450", "OD_540", "OD_570", "viability_1hr", "corrected", "loganswer", "conc pg/mL", "below Std Curve", "above Std Curve", "dilutionCorrected_conc", "Ro_conc")

write.xlsx(OD5, here("output", "CortisolELISAvalues.xlsx"), sheetName = "Sheet1", col.names = TRUE)

```


## Plotting Normalized Cortisol and Viability for both Stimulated/Unstimuated
...
p_cort_per_stim <- p_cort_per_stim + ylim(0,120)
p_cort_per_stim
ggsave(plot = p_cort_per_stim, filename = "./plots/Fig1_cort_percent_sd.pdf", width = 3, height = 4)
...
ggsave(plot = p_all_PR, filename = here("plots", "Fig1Supp_Stim_CortisolELISA.pdf"), width = 6, height = 3)
...
ggsave(plot = p_all_PR_us, filename = here("plots", "Fig1Supp_Unstim_CortisolELISA.pdf"), width = 6, height = 3)
```


## Calculating IC50 values for Stimulated cells


```{r IC50 values, warning=FALSE, message=FALSE}
#Fit for percent Cortisol
fit <- drm(formula =   percent ~ Ro_conc , data = OD5stim, fct=LL.4(names = c("Slope", "Lower Limit", "Upper Limit", "IC50")))
summary(fit)
plot(fit, type = "all")


#Fit for pPB
fit <- drm(formula =   pPB ~ Ro_conc , data = OD5stim, fct=LL.4(names = c("Slope", "Lower Limit", "Upper Limit", "IC50")))
summary(fit)
plot(fit, type = "all")
