...
fit <- drm(formula =   percent ~ Ro_conc , data = OD5stim, fct=LL.4(names = c("Slope", "Lower Limit", "Upper Limit", "ED50")))
summary(fit)
plot(fit, type = "all")
# This is a description of the variables and terms defined in the 'fit' model.
...
y <- (fit$coefficients[2]+ (fit$coefficients[3]- fit$coefficients[2])/(1+(x/fit$coefficients[4])^ fit$coefficients[1])) 
zz<- 
(fit$coefficients[2]+ (fit$coefficients[3]- fit$coefficients[2])/(1+(z/fit$coefficients[4])^ fit$coefficients[1]))

(fit$coefficients[3]+ (fit$coefficients[2]- fit$coefficients[3])/(1+(z/fit$coefficients[4])^ fit$coefficients[1]))
...
y <- (fit$coefficients[3]+ (fit$coefficients[2]- fit$coefficients[3])/(1+(z/fit$coefficients[4])^ fit$coefficients[1])) 
...
```{r playing with modeling, message=FALSE, warning=FALSE, echo=FALSE}
drm(percent_response~conc, data=toxdata, 
                  fct=LL.4(fixed=c(NA, 0, 100, NA),
                           names = c("Slope", "Lower Limit", "Upper Limit", "ED50")))

fit <- drm(formula =   pPB ~ Ro_conc , data = OD5stim,  
                  fct=LL.4(fixed=c(NA,0,100,NA),names = c("Slope", "Lower Limit", "Upper Limit","ED50")))

summary(fit)
plot(fit, type = "all")

mselect(fit, fctList = list(W1.3(fixed=c(NA, 100, NA)),W1.4(), W2.3(fixed=c(NA, 100, NA)), W2.4(),  LL.4()),linreg=TRUE) 
...
fit$coefficients[2]*(
 (((100-0)/(50-0))-1)^(1/ fit$coefficients[1]))
